...
def save_upload_file(PostFile, FilePath):
    try:
        sx3_obj = SX3()
        log.info("editor upload file to:{}".format(FilePath))
        success, upload_url = sx3_obj.upload(FilePath, PostFile)
        # success, upload_url = True, '/upload/test'
        if success:
            return u"SUCCESS", upload_url
        return u"写入文件失败", None
    except Exception, e:
        log.exception('Upload file error! file path:{}'.format(FilePath))
        return u"写入文件错误:" + e.message, None
...
def UploadFile(request):
    ...
    if state == "SUCCESS":
        if action == "uploadscrawl":
            state, uploaded_url = save_scrawl_file(request, os.path.join(OutputPath, OutputFile))
        else:
            #保存到文件中，如果保存错误，需要返回ERROR
            state, uploaded_url = save_upload_file(file, os.path.join(OutputPath, OutputFile))

    #返回数据
    return_info = {
        'url': uploaded_url,  # 保存后的文件名称
...
def save_scrawl_file(request, filename):
    ...
    try:
        ...
        decodestring = base64.decodestring(content)
        f = StringIO(decodestring)
        sx3_obj = SX3()
        success, upload_url = sx3_obj.upload(filename, f)
        if success:
            return "SUCCESS", upload_url
        return "写入图片文件失败", None
    except Exception, E:
        log.exception('Upload file error! file name:{}'.format(filename))
        return "写入图片文件错误:%s" % E.message, None
