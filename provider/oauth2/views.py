...
from .forms import PasswordGrantForm, RefreshTokenGrantForm, SocialOAuthGrantForm
...
class AccessTokenView(AccessTokenView):
    ...
    def get_social_oauth_grant(self, request, data, client):
        form = SocialOAuthGrantForm(data, client=client)
        if not form.is_valid():
            raise OAuthError(form.errors)
        return form.cleaned_data
